...
mod tests {
    ...
    fn get_context(signer_account_id: String, input: Vec<u8>, is_view: bool) -> VMContext {
        VMContext {
    ...
    fn set_get_permission_different_context() {
        let context = get_context("johan".to_string(), vec![], false);
        testing_env!(context);
        let mut contract = RepositoryPermission::default();
        assert_eq!(PERMISSION_FREE, contract.get_permission("johan".to_string(), "testrepo".to_string()));
        assert_eq!(true, contract.set_permission("testrepo".to_string(), "johan".to_string(), PERMISSION_OWNER));
        assert_eq!(true, contract.set_permission("testrepo".to_string(), "peter".to_string(), PERMISSION_CONTRIBUTOR));

        let context2 = get_context("peter".to_string(), vec![], false);
        testing_env!(context2);

        assert_eq!(PERMISSION_CONTRIBUTOR, contract.get_permission("peter".to_string(), "testrepo".to_string()));

        let context3 = get_context("johan".to_string(), vec![], false);
