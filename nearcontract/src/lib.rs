...
use near_sdk::{env, near, NearToken, PanicOnDefault};
...
/// Old state format for migration from the original contract
...
impl Contract {
    ...
    pub fn init() -> Self {
        ...
        // Try to read old (pre-migration) state format
        let old_state: Option<RepositoryPermission> = env::state_read();
        if let Some(old_state_value) = old_state {
    ...
    pub fn set_permission(
        ...
        assert_eq!(
            NearToken::from_yoctonear(10u128.pow(23)),
...
mod tests {
    ...
    const REQUIRED_ATTACHED_DEPOSIT: NearToken = NearToken::from_yoctonear(100000000000000000000000);
    ...
    fn set_get_permission_different_context() {
        ...
        let context2 = get_context("peter".to_string(), false, NearToken::from_near(0));
        ...
        let context3 = get_context("johan".to_string(), false, NearToken::from_near(0));
    ...
    fn require_attached_deposits() {
        let context = get_context("peter".to_string(), false, NearToken::from_yoctonear(2));
        testing_env!(context);
        let mut contract = Contract::init();
